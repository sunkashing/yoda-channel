...
function init_top(){
  ...
  function load_instagram(){
    ...
    $.ajax({
      type:'GET',
      url: api_instagram_url,
      dataType: "json"
    }).done(function(json) {
      console.log("json.data.length="+json.data.length);
      var load_cnt_max = 5;
      var load_cnt = json.data.length > load_cnt_max ? load_cnt_max : json.data.length;
      for(var i = 0; i < json.data.length; i++){
        $(".load_instagram").append('<div class="list_instagram" style="background-image:url('+json.data[i].images.standard_resolution.url+');"><a href="'+json.data[i].link+'" target="_blank"></a></li>');
        console.log("i="+i);
        if( i > (load_cnt_max + 1 )){
          break;
        }
